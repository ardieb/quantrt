...
class Scheduler(object):
    ...
    def __init__(self, logger: Optional[Logger] = None, executor: Optional[Executor] = None) -> None:
        self.jobs: List[Job] = []
        self.logger: Optional[Logger] = logger
        self.executor: Optional[Executor] = executor

    def run_pending(self, blocking: bool = True) -> None:
        """
        ...
        :param blocking: Whether this should block the calling thread. Can only be False if
        the executor is not `None`.
        """
        runnable_jobs = (job for job in self.jobs if job.should_run)
        for job in sorted(runnable_jobs):
            self._run_job(job, blocking)
    ...
    def _run_job(self, job: "Job", blocking: bool = True) -> None:
        ret = job.run(executor=self.executor, blocking=blocking)
...
class Job(object):
    ...
    * a quantity of `time units` defined by `interval`
    ...
    def __init__(self, interval: int, scheduler: Scheduler = None):
        ...
        # event to schedule on `market_open` or `market_close`
        self.event: Optional[str] = None

        # before event
        self.before_event_dt: Optional[datetime.timedelta] = None

        # after event
        self.after_event_dt: Optional[datetime.timedelta] = None
    ...
    def at_event(self, event: str) -> "Job":
        """
        Specify an event to run at.
        :param event: A string representing the event to schedule around.
        Must be one of `market_open/<exchange>` or `market_close/<exchange>`.
        :return: The job instance
        """
        if not event.startswith("market_open") and not event.startswith("market_close"):
            raise ScheduleValueError(
                "Invalid event ({} is not one of `market_open` or `market_close`)".format(event)
            )
        self.event = event
        self.unit = "event"
        return self

    def before_event(self, event: str, seconds: int = 0, minutes: int = 0, days = 0) -> "Job":
        """
        Schedule the job to run before an event.
        :param event: String representation of the event to schedule around
        :param seconds: The number of seconds to schedule the job before. Defaults to 0.
        :param minutes: The number of minutes to schedule the job before. Defaults to 0.
        :param days: The number of days to schedule the job before. Defaults to 0.
        """
        self = self.at_event(event)
        self.before_event_dt = datetime.timedelta(seconds=seconds, minutes=minutes, days=days)
        return self

    def after_event(self, event: str, seconds: int = 0, minutes: int = 0, days = 0) -> "Job":
        """
        Schedule the job to run after an event.
        :param event: String representation of the event to schedule around
        :param seconds: The number of seconds to schedule the job after. Defaults to 0.
        :param minutes: The number of minutes to schedule the job after. Defaults to 0.
        :param days: The number of days to schedule the job after. Defaults to 0.
        """
        self = self.at_event(event)
        self.after_event_dt = datetime.timedelta(seconds=seconds, minutes=minutes, days=days)
        return self
    ...
    def run(self, executor: Optional[Executor] = None, blocking: bool = True):
        """
        ...
        :param executor: The optional executor to run the job on.
        :param blocking: Whether the job should wait on the result from the executor. Executor must
        not be none.
        :return: The return value returned by the `job_func`, or CancelJob if the job's
                 deadline is reached.
        """
        if self._is_overdue(datetime.datetime.now()):
            if self.scheduler.logger: self.scheduler.logger.debug("Cancelling job %s", self)
            return CancelJob

        if self.scheduler.logger: self.scheduler.logger.debug("Running job %s", self)

        if executor:
            ret = executor.submit(self.job_func)
            if blocking:
                ret = ret.result()
        else:
            ret = self.job_func()
    ...
    def _schedule_next_run(self) -> None:
        ...
        """
        if self.event:
            event, exchange = self.event.split("/")
            cal: exchange_calendars.ExchangeCalendar = exchange_calendars.get_calendar(exchange)
            if event == "market_open":
                dt: datetime.datetime = cal.next_open(pd.Timestamp.today(tz=pytz.UTC)).to_pydatetime()
                dt = dt.astimezone(tz=EST).replace(tzinfo=None)
            elif event == "market_close":
                dt: datetime.datetime = cal.next_close(pd.Timestamp.today(tz=pytz.UTC)).to_pydatetime()
                dt = dt.astimezone(tz=EST).replace(tzinfo=None)
            else:
                raise ScheduleValueError(
                    "Invalid event ({} is not of type `market_open` or `market_close`)".format(event)
                )
            if self.before_event_dt:
                self.next_run = dt - self.before_event_dt
            elif self.after_event_dt:
                self.next_run = dt + self.after_event_dt
            else:
                self.next_run = dt
            return
    ...
    def _decode_datetimestr(
        ...
        for f in formats:
            try:
                ...
                if self.scheduler.logger: self.scheduler.logger.exception("Unable to format datetime")
        return None
